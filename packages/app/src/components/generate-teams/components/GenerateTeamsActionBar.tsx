...
export function GenerateTeamsActionBar({ onRedraw, onSave }: GenerateTeamsActionBarProps): React.JSX.Element {
  return (
    <View className="absolute bottom-0 left-0 right-0 flex-row items-center gap-3 px-4 pb-8 pt-4 bg-surface/80 rounded-t-[24px] shadow-sm">
      <Pressable
        onPress={onRedraw}
        className="flex-1 flex-row items-center justify-center gap-2 py-4 rounded-xl border-2 border-outline-variant active:scale-[0.98]"
      >
        <RefreshCw size={16} color="#595c5d" />
        <Text className="text-sm font-headline font-bold text-on-surface">
          Refazer Sorteio
        </Text>
      </Pressable>
      <Pressable
